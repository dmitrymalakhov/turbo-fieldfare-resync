...
@Suite struct AppModelHistoryTests {
    @MainActor @Test func nativeDeletionAlsoRemovesTheForkArchiveAcrossRelaunch() async throws {
        let (model, root) = try await readyModel(FakeInferenceClient(eventDelay: .milliseconds(1)))
        defer { try? FileManager.default.removeItem(at: root) }
        model.promptText = "delete this exchange"
        model.send()
        try await finish(model)
        let storedID = try #require(model.storedConversationID)
        let chatID = model.selectedChatID
        model.deleteConversation(id: storedID)
        await model.conversationDeletionTask?.value
        model.flushChatPersistence()
        #expect(!model.chats.contains { $0.id == chatID })
        #expect(!model.chats.contains { $0.conversationID == storedID })
        let archive = AppChatFileStore.loadOrCreateWithRecovery(
            forModelDirectory: URL(fileURLWithPath: model.modelPathText)).archive
        #expect(!archive.chats.contains { $0.id == chatID })
        #expect(archive.chats.allSatisfy { $0.messages.isEmpty })
    }

    @MainActor @Test func clearingStoredHistoryCanUndoWithoutRevivingDeletedTokenRecords() async throws {
        let (model, root) = try await readyModel(FakeInferenceClient(eventDelay: .milliseconds(1)))
        defer { try? FileManager.default.removeItem(at: root) }
        model.promptText = "keep this exchange for undo"
        model.send()
        try await finish(model)
        let storedID = try #require(model.storedConversationID)
        let messages = model.selectedChat.messages
        model.clearOutput()
        await model.conversationDeletionTask?.value
        #expect(model.selectedChat.messages.isEmpty)
        #expect(model.canUndoClearHistory)
        model.undoClearHistory()
        #expect(model.selectedChat.messages == messages)
        #expect(model.selectedChat.conversationID == nil)
        #expect(model.history.entry(storedID) == nil)
        model.promptText = "continue after undo"
        model.send()
        try await finish(model)
        #expect(model.storedConversationID != storedID)
