...
public actor MultimodalConversation {
    ...
    public func send(
        ...
        openingMessages: [MultimodalMessage]? = nil
        ...
        return try await generate(
            ...
            onProgress: onProgress, openingMessages: openingMessages)
    }
    ...
    private func encodeTurn(
        parts: [MultimodalContinuationPart],
        images: [URL],
        openingMessages: [MultimodalMessage]?,
        checkCancellation: @Sendable () throws -> Void
    ) async throws -> EncodedTurn {
        if kvTokenIDs.isEmpty, let openingMessages {
            let ids = openingMessages.flatMap { message in
            ...
            if !images.isEmpty {
                ...
                for (id, url) in zip(ids, images) {
                    try checkCancellation()
                    features[id] = try visionRuntime.encodeImage(at: url,
                        languageModel: model, residencyPolicy: visionResidency,
                        checkCancellation: checkCancellation)
                }
            }
            let input = try MultimodalPromptRenderer.render(messages: openingMessages,
                featuresByID: features, tokenizer: tokenizer)
            return EncodedTurn(effectiveTokenIDs: input.effectiveTokenIDs,
                               prefillInput: images.isEmpty ? nil : input)
        }
