...
actor RealInferenceSession {
        ...
        report: @escaping @Sendable (RawDecodeProgress) -> Void
            ...
            if request.imageAttachments.isEmpty {
                ...
                let messages = request.messages.enumerated().map { index, message in
                    ...
                    let ids = message.imageIDs ?? (index == lastUserIndex
                        ? request.imageAttachments.map(\.id) : [])
                    content.append(contentsOf: ids.map { .image(id: $0) })
    ...
    /// Full history is used only for an empty conversation. Subsequent turns
    /// resume the recorded KV and use the new user message alone.
    static func openingMessages(for request: AppGenerationRequest) -> [MultimodalMessage]? {
        guard request.messages.count > 1 else { return nil }
        let last = request.messages.count - 1
        return request.messages.enumerated().map { index, message in
            let ids = message.imageIDs ?? (index == last ? request.imageAttachments.map(\.id) : [])
            var parts = ids.map { MultimodalContentPart.image(id: $0) }
            if !message.content.isEmpty { parts.append(.text(message.content)) }
            return MultimodalMessage(role: tokenizerMessage(message).role, content: parts)
        }
    }
    ...
    private func runConversationTurn(
        ...
        do {
            ...
            let turn = try await conversation.send(
                ...
                shouldStop: stopFlagReader(),
                onProgress: report,
                openingMessages: Self.openingMessages(for: request))
