...
extension AppModel {
    ...
    private func refreshHistory(store: ConversationStore, generation: UInt64) async {
        ...
        registerStoredConversations(entries)
    ...
    func ensureStoredConversation(firstMessage: String) async -> UUID? {
        ...
        do {
            ...
            if let chatID = activeRunChatID,
               let index = chats.firstIndex(where: { $0.id == chatID }) {
                chats[index].conversationID = meta.id
                persistChats()
            }
    ...
    public func openConversation(id: UUID) {
        ...
        selectChatForStoredConversation(id)
    ...
    private func loadDocument(id: UUID, renderID: UUID) {
        ...
        Task { [weak self] in
            ...
            if let document = screen.document { projectStoredDocument(document) }
    ...
    public func deleteConversation(id: UUID) {
        ...
        let store = binding.store
        let deletedChatIDs = Set(chats.filter { $0.conversationID == id }.map(\.id))
        let generation = binding.generation
        conversationDeletionTask = Task { [weak self] in
            ...
            do {
                ...
                forgetStoredChats(deletedChatIDs)
